...
| R9  | 주문에 쿠폰 1장을 적용할 수 있으며, 적용 전 금액·할인액·최종 결제액을 스냅샷으로 보존한다 | System |

> 쿠폰 적용 규칙·할인 계산은 아래 **[공통] 쿠폰(Coupon)** 섹션 참조.
...
- 주문 흐름은 재고 행을 **비관적 락(`SELECT … FOR UPDATE`)으로 조회 → `Stock.reduce(qty)`** 차감한다 (Facade 단일 트랜잭션, ADR-0001). 재고 부족이면 `재고가 부족합니다.`(BAD_REQUEST)로 주문 전체 롤백. 동시 차감은 락으로 직렬화되어 음수·Lost Update가 발생하지 않는다. *(명백 품절을 락 진입 전 빠르게 거르는 `isAvailable` 사전체크 = ADR-0001 L1 — 핫 경합 시 도입하는 선택적 최적화이며 현재 v1엔 미적용)*
...
| R9  | 주문 시 재고 행을 비관적 락으로 조회 후 차감하며, 재고 부족(0 포함) 시 R2(재고 부족) 흐름으로 주문 전체가 실패한다. 동시 차감은 락으로 직렬화 | System |
...
**담당:** 운영이 할인 정책(템플릿)을 정의하고, 유저가 발급받은 쿠폰을 주문에 1장 적용해 할인받는다. 한 번 사용한 쿠폰은 재사용 불가.

**핵심 개념:**
- **두 개념을 분리한다**: **쿠폰 템플릿**(운영 소유, 할인 정책)과 **발급된 쿠폰(UserCoupon)**(유저 소유, 인스턴스).
- 템플릿은 타입(`FIXED` 정액 / `RATE` 정률), 할인값, 최소주문금액(선택), 만료일을 가진다.
- 발급된 쿠폰은 상태(`AVAILABLE` / `USED`)를 가지며, **만료(`EXPIRED`)는 `expiredAt`과 현재 시각 비교로 조회 시 파생**한다 (별도 저장·배치 없음).
- 주문 1건에 쿠폰은 **최대 1장**. 주문 성공 시 즉시 `USED`로 전이되어 재사용 불가.
- 주문에는 **쿠폰 적용 전 금액 · 할인 금액 · 최종 결제 금액**을 스냅샷으로 보존한다.

**할인 계산 정책 (회사 손해 방지 원칙):**
- 정률(`RATE`): 할인액 = `floor(주문금액 × value / 100)` — **소수점 버림**(1원도 더 깎지 않음).
- 정액(`FIXED`): 할인액 = `min(value, 주문금액)` — 주문액 초과분 환급 없음.
- 최종 결제액 = `주문금액 − 할인액`, **0원 하한**(음수 불가).
- 금액은 원 단위 정수 계산(코드상 `Double` 저장이나 계산 단계마다 원 단위 버림으로 정산 오차 차단).

**비즈니스 규칙:**

| #  | 규칙                                                                                                   | Actor  |
|----|--------------------------------------------------------------------------------------------------------|--------|
| R1 | 쿠폰 템플릿은 정액(`FIXED`) 또는 정률(`RATE`) 타입을 가진다                                             | Admin  |
| R2 | 유저는 템플릿으로부터 쿠폰을 발급받아 소유한다                                                          | User   |
| R3 | 주문에는 쿠폰을 최대 1장 적용한다                                                                       | User   |
| R4 | 미존재·타 유저 소유·이미 사용(`USED`)·만료·최소주문금액 미달 쿠폰으로 요청 시 **주문 전체가 실패한다**  | System |
| R5 | 주문 성공 시 해당 쿠폰은 즉시 `USED`로 전이되어 재사용 불가하다                                         | System |
| R6 | 같은 쿠폰의 동시 사용은 비관적 락으로 직렬화되어 1건만 성공한다                                         | System |
| R7 | 주문은 적용 전 금액·할인 금액·최종 결제 금액을 스냅샷으로 보존한다                                      | System |
| R8 | 만료 상태(`EXPIRED`)는 `expiredAt < now`로 조회 시 동적 판정한다                                        | System |

**미결 사항 (공동 결정 필요):**
- 한 유저의 동일 템플릿 중복 발급 허용 여부 / 템플릿별 총 발급 수량 한도
- 정률 쿠폰 최대 할인 한도(cap) 도입 여부 (현재 미적용)

**확장 포인트 (현 범위 외):**
- 웰컴 쿠폰 자동 발급(`UserRegistered` 트리거), 포인트와의 분할 적용
...
> **쿠폰은 MVP 범위로 이동**(위 `[공통] 쿠폰(Coupon)` 섹션 참조). 포인트·행동 기록만 현재 범위 외 — 경계와 트리거만 정의한다.
